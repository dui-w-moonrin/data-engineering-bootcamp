...
def _load_data_to_gcs():
    ...
    storage_client.bucket(BUCKET_NAME).blob(destination_blob_name).upload_from_filename(
        str(file_path)
    )
...
with DAG(
    ...
    schedule="@daily",
    catchup=False,
    tags=["DEB", "Skooldio", "greenery", "addresses"],
) as dag:
    ...
    transform_data = SparkSubmitOperator(
        ...
        conn_id="my_spark",
        name="greenery_addresses_transform",
        verbose=True,
    )
